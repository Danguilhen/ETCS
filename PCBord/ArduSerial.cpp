...
void WindowsSerial::begin(unsigned int _baud)
{
	...
	if (this->comPortNum != 0)
	{
		char portName[20];

		snprintf(portName, 12, "\\\\.\\COM%i", this->comPortNum);
...
bool WindowsSerial::print(const double message)
{
	DWORD bytesSend;
	char buffer[100];

	unsigned int bufSize = sprintf_s(buffer, "%f", message);
...
bool WindowsSerial::print(const long message)
{
	...
	unsigned int bufSize = sprintf_s(buffer, "%ld", message);
...
bool WindowsSerial::println(const double message)
{
	bool result = this->print(message);
	if (result)
		result = this->write('\n');
