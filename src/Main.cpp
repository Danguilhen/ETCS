...
int main()
{
    ...
    RenderWindow app(VideoMode(800, 600, 32), "Pouet !");
    app.setFramerateLimit(60);

    Shape courbe = Shape::Line(0, 300, 800, 300, 1, Color(255, 0, 0));
    // Ajoute un point par pixel sur la courbe
    for(int i = 1; i < 79; i++)
    {
        courbe.AddPoint((i*10), 300, Color(255, 0, 0));
    }
    ...
    while (app.IsOpened())
    {
        ...
        // Décale tous les points vers la gauche
        for(int i = 0; i < 80; i++)
        {
            courbe.SetPointPosition(i, (i*10), courbe.GetPointPosition(i+1).y);
        }

        // Modifie la position du point 400 au coordonnées (400, partie entière de 300*resultat)
        courbe.SetPointPosition(41, 400, ceil(300*resultat));
