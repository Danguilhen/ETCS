...
{
	//cout << v_train->etcs.Bord.TrackRI.getPointKilometrique()/1000 - float(v_train->etcs.Bord.TrainRI.T_data.getTrain_length()/1000.0) << endl;
	size_t i = 0;
	for (i = 0; i < liste_canton.size(); i++)
	{
		...
		|| (v_train->etcs.Bord.TrackRI.getPointKilometrique()/1000 - float(v_train->etcs.Bord.TrainRI.T_data.getTrain_length()/1000.0) >= liste_canton[i]["PK_debut"] && v_train->etcs.Bord.TrackRI.getPointKilometrique()/1000 - float(v_train->etcs.Bord.TrainRI.T_data.getTrain_length()/1000.0) < liste_canton[i]["PK_fin"] ))
		{
